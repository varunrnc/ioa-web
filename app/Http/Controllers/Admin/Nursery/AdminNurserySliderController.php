...
class AdminNurserySliderController extends Controller
{
    private $path = "public/img/slider/nursery/";
    public function index(Request $req)
    {

        $datalist = null;
        if ($req->q != null) {
            $datalist = NurserySlider::where('slider_name', 'LIKE', '%' . $req->q . '%')
                ->orWhere('title', 'LIKE', '%' . $req->q . '%');
            return view('admin.slider.nursery.index', compact('datalist'));
        } else {
            $datalist =  NurserySlider::orderBy('order_no', 'ASC');
    ...
    {
        return view('admin.slider.nursery.create');
    }
    public function save(Request $req)
    {
        ...
        $orderNo = NurserySlider::max('order_no') + 1;
        $obj = new NurserySlider();
    ...
    {
        $data = NurserySlider::Where('id', $req->id)->first();
        return view('admin.slider.nursery.edit', compact('data'));
    }
    public function  update(Request $req)
    {
        ...
        $obj = NurserySlider::Where('id', $req->id)->first();
        $obj->slider_name = $req->slider_name;
        $obj->title = $req->title;
        $obj->description = $req->description;
        $obj->button_name = $req->button_name;
    ...
    public function reorder(Request $req)
    {
        ...
        $status =  $obj->update();
        $datalist =  NurserySlider::where('order_no', '>', $req->order_no)->orderBy('order_no')->get();
    ...
    public function delete(Request $req)
    {
        ...
        $status = NurserySlider::where('id', $req->id)->delete();
        if ($status) {
