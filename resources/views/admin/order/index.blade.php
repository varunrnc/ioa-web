    ...
    <div class="admin-container">
        ...
        <div class="cart__table">
            <table class="cart__table--inner">
                {!! Hpx::table_headings(['Order', 'Invoice No.', 'Date', 'Payment', 'order-status', 'action:text-right']) !!}
                <tbody class="cart__table--body" id="slider-list">
                    @foreach ($data_list as $data)
                        <tr class="cart__table--body__items">
                                    ...
                                    <div class="cart__content">
                                        <span class="cart__content--variant fw-bold">
                                            <h3 class="cart__content--title text-capitalize">
                                                <a href="{{route($route_name . '.show', $data->id)}}">{{ strtolower($data->username) }}</a>
                                            </h3>
                            ...
                            <td class="cart__table--body__list">
                                <span class="text-capitalize payment-status py-{{$data->payment_status}}">{{$data->payment_status}}</span>
                                <span class="ms-2">
                                    <i class="icofont-refresh payment-refresh" data-id="{{$data->id}}" data-payment-id="{{$data->payment_id}}"></i>
                                    {!! Hpx::spinner() !!}
                                </span>
                            </td>
                            <td class="cart__table--body__list">
...
@section('style')
    <style type="text/css">
        ...
        .payment-refresh:hover{ color: #0d6efd; cursor: pointer;}
