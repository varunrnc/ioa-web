...
@section('main-content')
    ...
    <div class="admin-container">
        ...
        @php
            ...
            $tbx['title'] = 'Plants';
            $tbx['btn-name'] = 'Add';
            $tbx['btn-link'] = route('admin.mplant.create');
            $tbx['search-bar'] = true;
                ...
                <tbody class="cart__table--body" id="slider-list">
                    @foreach ($data as $item)
                        <tr class="cart__table--body__items">
                            <td class="cart__table--body__list">
                                <div class="cart__product d-flex align-items-center">
                                    <div class="cart__thumbnail">
                                        <img src="{{ url($item->img) }}" class="border-radius-5" alt="Category">
                                    </div>
                                    <div class="cart__content">
                                        <h3 class="cart__content--title h4"><a href="#">{{ $item->title }}</a></h3>
                                        <span
                            ...
                            <td class="cart__table--body__list">
                                <h3 class="cart__content--title h4"><a href="#">{{ $item->sub_category }}</a></h3>
                            </td>
                            ...
                            <td class="cart__table--body__list text-right">
                                <a class="btn btn-sm fs-4 btn-outline-secondary edit__btn"
                                    href="{{ url('admin/mplant/edit') . '/' . $item->id }}">Edit</a>
                                <a id="{{ $item->id }}" href="#"
...
@section('script')
    <script>
        ...
        function status(id) {
            ...
            let req = api.setData("{{ route('admin.mplant.status') }}", data);
            req.then((res) => {
                if (res.status == true) {
        ...
        function deleteItem(id) {
            ...
            let req = api.setData("{{ route('admin.mplant.delete') }}", data);
