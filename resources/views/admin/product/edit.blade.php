...
<form class="row" action="{{route($route_name.'.store')}}" enctype="multipart/form-data" id="myForm">
	...
	<div class="col-12">
		<div class="row">
			...
			<div class="col-sm-2">
				<label for="" class="form-label">OGD No.</label>
				<input type="text" class="form-control" name="ogd_no" value="{{$data->ogd_no}}" placeholder="Hint - OGD1..2..3..">
			</div>
			<div class="col-sm-2">
			...
			<div class="col-sm-3">
				<label for="" class="form-label">Shipping Charge ( default
					<input type="checkbox" name="shipping_checkbox" value="{{Hpx::shipping_charge()}}" data-current-value="{{$data->shipping_charge}}"
					{{Hpx::shipping_charge() == $data->shipping_charge ? 'checked':null}}
					> )
				</label>
				<input type="text" class="form-control" name="shipping_charge" value="{{$data->shipping_charge ?? '0.00'}}"
				{{Hpx::shipping_charge() == $data->shipping_charge ? 'readonly':null}}
				>
			</div>		
			<div class="col-sm-3">
...
<script>
	...
	$(document).ready(function(){
		...
		$('input[name="shipping_checkbox"]').click(function(){
			if($(this).prop('checked') == true){
				$('input[name="shipping_charge"]').val($(this).val()).prop('readonly',true);
			}else{
				$('input[name="shipping_charge"]').val($(this).attr('data-current-value')).prop('readonly',false);
			}
		});
	});
