...
@section('main-content')
    ...
    <div class="admin-container">
        ...
        @php
            $tbx['tb'] = 1;
            $tbx['title'] = 'Nursery Slider List';
            $tbx['btn-name'] = 'Add';
            $tbx['btn-link'] = route('admin.nursery.slider.create');
            $tbx['search-bar'] = true;
                            ...
                            <td class="cart__table--body__list text-right">
                                <div class="btn-groupx" role="group" aria-label="Action">
                                    <a class="btn btn-sm fs-4 btn-outline-secondary"
                                        href="{{ route('admin.nursery.slider.edit', $data->id) }}">Edit</a>
...
@section('script')
    <script>
        ...
        function status(id) {
            ...
            let req = api.setData("{{ route('admin.nursery.slider.status') }}", data);
            req.then((res) => {
                if (res.status == true) {
        ...
        function reorder(event, id) {
            if (event.keyCode == 13) {
                ...
                let req = api.setData("{{ route('admin.nursery.slider.reorder') }}", data);
                req.then((res) => {
                    if (res.status == true) {
        ...
        function deleteItem(id) {
            ...
            let req = api.setData("{{ route('admin.nursery.slider.delete') }}", data);
